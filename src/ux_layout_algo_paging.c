...
static const bagl_element_t ux_layout_algo_paging_elements[] = {
  // erase
...
static const bagl_element_t* ux_layout_algo_paging_prepro(const bagl_element_t* element) {
  // don't display if null
  const ux_layout_algo_paging_params_t* params = (const ux_layout_algo_paging_params_t*)ux_stack_get_current_step_params();
...
static unsigned int ux_layout_algo_paging_button_callback(unsigned int button_mask, unsigned int button_mask_counter);
...
unsigned int ux_layout_algo_paging_compute(unsigned int stack_slot, unsigned int page_to_display) {
  const ux_layout_algo_paging_params_t* params = (const ux_layout_algo_paging_params_t*)ux_stack_get_step_params(stack_slot);
...
void ux_layout_algo_paging_redisplay(unsigned int stack_slot) {

  // request offsets and lengths of lines for the current page
  ux_layout_algo_paging_compute(stack_slot, G_ux.layout_paging.current);

  G_ux.stack[stack_slot].element_arrays[0].element_array = ux_layout_algo_paging_elements;
  G_ux.stack[stack_slot].element_arrays[0].element_array_count = ARRAYLEN(ux_layout_algo_paging_elements);
  G_ux.stack[stack_slot].element_arrays_count = 1;
  G_ux.stack[stack_slot].screen_before_element_display_callback = ux_layout_algo_paging_prepro;
  G_ux.stack[stack_slot].button_push_callback = ux_layout_algo_paging_button_callback;
  ux_stack_display(stack_slot);
}

static void ux_layout_algo_paging_next(void) {
  if (G_ux.layout_paging.current == G_ux.layout_paging.count-1) {
    ...
    ux_layout_algo_paging_redisplay(G_ux.stack_count-1);
  }
}

static void ux_layout_algo_paging_prev(void) {
...
static unsigned int ux_layout_algo_paging_button_callback(unsigned int button_mask, unsigned int button_mask_counter) {
  ...
  switch(button_mask) {
    case BUTTON_EVT_RELEASED|BUTTON_LEFT:
      ux_layout_algo_paging_prev();
      break;
    case BUTTON_EVT_RELEASED|BUTTON_RIGHT:
      ux_layout_algo_paging_next();
      break;
...
void ux_layout_algo_paging_init(unsigned int stack_slot) {
  ...
  G_ux.layout_paging.count = ux_layout_algo_paging_compute(stack_slot, -1UL); // at least one page
  ...
  ux_layout_algo_paging_redisplay(stack_slot);
}
